...
// キーワードとして追加している。
// 「Ｊ１広島は１７日、ＦＷ中村草太（２３）がフランス１部ルアーブルに
// 期限付き移籍することを発表した」のようなサンフレッチェ広島の記事が、
// クラブ名を「サンフレッチェ」ではなく「Ｊ１広島」(Ｊ1リーグ+地名)と略す
// ケースがあり、既存の「サッカー」「Jリーグ」「サンフレッチェ」キーワード
// のいずれにも該当せず素通りしていた(正規化後は「J1広島」となり「Jリーグ」
// 完全一致もしない)。「J1」「J2」「J3」というJリーグの所属ディビジョン
// 表記は野球記事でまず使われない語彙なので追加している。あわせて
// 「期限付き移籍」(=いわゆるレンタル移籍)もJリーグ・海外サッカー特有の
// 制度用語で、NPBにはこの表現の移籍制度自体が存在しないため、球団名
// (「広島」等)を伴わない記事でも確実に検知できる安全な追加キーワードとして
// 使える。
// 「【NFL】運動能力も強さも向上した2年目WRテスラーは「まったく別人」と
// ライオンズWRコーチ」のようなNFL(アメリカンフットボール)記事が、
// デトロイト・ライオンズの「ライオンズ」に反応して西武ライオンズと誤判定
// される事例が確認された。「ライオンズ」は西武のstrongKeywords(正式名称・
// 愛称)側に入っており、strongKeywordsは野球文脈の有無を問わず無条件で
// ヒットする仕様のため、shortKeywords用の文脈チェックでは救えない。
// この種の衝突は「NBA」「ホークス」の時と同じ構造で、除外キーワード側で
// 競技そのものを検知して先に弾く必要がある。
// 「鹿島、神戸DF広瀬陸斗の獲得決定的 濃野公人は米MLS移籍間近 元横浜Mの
// ヤン・マテウスも獲得へ(スポニチアネックス)」のようなJリーグ移籍記事が、
// クラブ名を「マリノス」ではなく「横浜M」と略しているせいでOTHER_SPORTS_
// KEYWORDSのどれにも該当せず素通りし、「横浜」がDeNAベイスターズの略称
// (shortKeywords)として誤ヒットした事例が確認された。この記事は米国の
// サッカーリーグ「MLS」への言及を含んでおり、MLSはNPB記事ではまず使われ
// ない語彙なので、クラブ名の略記ゆれに左右されない安全な追加キーワードと
// して使える。
...
// 「【セパ公示】（１８日）巨人がバルドナード、オリックスが吉田輝星ら３投手を
// 登録(スポーツ報知)」/本文「◆ＪＥＲＡ セ・リーグ 【出場選手登録】 ▽巨人
// バルドナード投手 【出場選手登録抹消】 なし ◆パ・リーグ 【出場選手登録】
// ▽日本ハム…▽オリックス…」という第4の定型フォーマット(スポーツ報知)が
// 確認された。見出し先頭の括弧が「◯日のプロ野球公示」ではなく「セパ公示」
// (日付は次の括弧「（１８日）」に分離)なので既存のROSTER_TRANSACTION_TITLE_
// BRACKET_REに該当せず、本文の見出しも「【登録】」「【抹消】」ではなく
// 「【出場選手登録】」「【出場選手登録抹消】」(間に「出場選手」が挟まる)
// なので既存の本文マーカーとも一致しなかった。結果、複数球団(巨人・日本ハム・
// オリックス等)が本文中の言及だけで無条件にヒットし(セ・リーグ/パ・リーグ
// という一般キーワードにより野球文脈ありと判定されるため、曖昧な球団略称も
// 拘束なくマッチしてしまう)、事務的な公示記事が特定球団のページに紛れ込む
// 事例が確認された。
const ROSTER_TRANSACTION_TITLE_RE = /[／\/]\s*\d{1,2}日公示/;
const ROSTER_TRANSACTION_TITLE_BRACKET_RE = /^[【\[](?:\d{1,2}日のプロ野球公示|セパ公示|セ・パ公示)[】\]]/;
